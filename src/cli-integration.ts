...
program
  ...
  .action(async (options) => {
    try {
      ...
      // イベントリスナーの設定
      integrationService.on('sessionImported', (data) => {
        console.log(`📥 セッションインポート: ${data.sessionId}`)
      })
...
program
  ...
  .action(async (options) => {
    try {
      ...
      console.log('\n📈 ステータス情報:')
      console.log(`  監視状態: ${status.isWatching ? '✅ 監視中' : '❌ 停止中'}`)
      console.log(`  Cursorパス: ${status.cursorPath}`)
      console.log(`  最終スキャン: ${status.lastScan ? status.lastScan.toLocaleString() : '未実行'}`)
      console.log(`  発見セッション数: ${status.sessionsFound}`)
