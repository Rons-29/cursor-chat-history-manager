...
program
  ...
  .action(async (options) => {
    try {
      ...
      const session = await historyService.createSession(options.title, metadata);
...
program
  ...
  .action(async (options) => {
    try {
      ...
      console.log(`🔍 検索結果: ${result.totalCount}件中${result.sessions.length}件を表示`);
...
program
  ...
  .action(async () => {
    try {
      ...
      console.log(`   平均メッセージ数/セッション: ${stats.averageMessagesPerSession.toFixed(1)}`);
      console.log(`   ストレージサイズ: ${(stats.storageSize / 1024 / 1024).toFixed(2)} MB`);
      
      if (stats.oldestSession) {
        console.log(`   最古のセッション: ${stats.oldestSession.toLocaleString()}`);
      }
      if (stats.newestSession) {
        console.log(`   最新のセッション: ${stats.newestSession.toLocaleString()}`);
      }
    } catch (error) {
...
program
  ...
  .action(async () => {
    try {
      ...
      console.log(`🧹 クリーンアップ完了: ${deletedCount}個のセッションを削除しました`);
    } catch (error) {
  ...
  .action(async (options) => {
    try {
      ...
      if (!options.force) {
        ...
        console.log(`   タイトル: ${session.title}`);
        console.log(`   メッセージ数: ${session.messages.length}`);
        console.log('   削除を続行するには --force オプションを使用してください');
        return;
      }
...
program
  ...
  .requiredOption('-f, --format <format>', 'エクスポート形式 (json|markdown|txt)')
  ...
  .action(async (options) => {
    try {
      ...
      console.log(`   総メッセージ数: ${sessions.reduce((sum, session) => sum + session.messages.length, 0)}`);
    } catch (error) {
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const result = await historyService.importSessions(options.file, importOptions);
...
program
  ...
  .action(async (options) => {
    try {
      ...
      console.log(`   ファイルサイズ: ${(result.size / 1024 / 1024).toFixed(2)} MB`);
    } catch (error) {
  ...
  .action(async (options) => {
    try {
      ...
      if (!options.force) {
        ...
        console.log('   復元を続行するには --force オプションを使用してください');
        return;
      }
  ...
  .action(async () => {
    try {
      ...
      backups.forEach((backup, index) => {
        ...
        console.log(`   ファイルサイズ: ${(backup.size / 1024 / 1024).toFixed(2)} MB`);
    ...
    try {
      ...
      if (options.set) {
        ...
        if (!key || value === undefined) {
          console.error('❌ 設定形式が正しくありません。例: --set maxSessions=500');
          process.exit(1);
        }
      ...
      console.log(`   バックアップ機能: ${config.enableBackup ? '有効' : '無効'}`);
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const startDate = options.startDate ? new Date(options.startDate) : new Date(endDate.getTime() - days * 24 * 60 * 60 * 1000);
      
      if (options.keywords) {
        console.log('🔍 キーワード分析:');
        const keywords = await analyticsService.analyzeKeywords(startDate, endDate);
      ...
      if (options.report) {
        ...
        console.log(`📈 ${period === 'daily' ? '日別' : period === 'weekly' ? '週別' : '月別'}活動レポート:`);
        
        const report = await analyticsService.generateActivityReport(startDate, endDate, period);
        ...
        console.log(`   平均セッション長: ${report.summary.averageSessionLength.toFixed(1)}メッセージ`);
        console.log(`   平均セッション時間: ${report.summary.averageSessionDuration.toFixed(1)}分`);
        
        console.log('\n📈 トレンド:');
        console.log(`   セッション数: ${report.trends.sessionTrend === 'increasing' ? '📈 増加' : report.trends.sessionTrend === 'decreasing' ? '📉 減少' : '➡️ 安定'}`);
        console.log(`   メッセージ数: ${report.trends.messageTrend === 'increasing' ? '📈 増加' : report.trends.messageTrend === 'decreasing' ? '📉 減少' : '➡️ 安定'}`);
        console.log(`   エンゲージメント: ${report.trends.engagementTrend === 'increasing' ? '📈 増加' : report.trends.engagementTrend === 'decreasing' ? '📉 減少' : '➡️ 安定'}`);
        
        console.log('\n📅 期間別詳細:');
        report.activities.slice(-10).forEach(activity => {
          console.log(`   ${activity.date}: ${activity.sessionCount}セッション, ${activity.messageCount}メッセージ`);
        });
        return;
      }
      ...
      console.log(`   総セッション数: ${stats.totalSessions}`);
      console.log(`   総メッセージ数: ${stats.totalMessages}`);
      console.log(`   平均セッション長: ${stats.averageSessionLength.toFixed(1)}メッセージ`);
      console.log(`   平均セッション時間: ${stats.averageSessionDuration.toFixed(1)}分`);
...
program
  ...
  .action(async () => {
    try {
      ...
      console.log(`   最終保存時刻: ${status.lastSaveTime ? status.lastSaveTime.toLocaleString() : 'なし'}`);
    } catch (error) {
...
program
  ...
  .action(async (options) => {
    try {
      ...
      if (options.idleTimeout) updates.idleTimeout = parseInt(options.idleTimeout);
      if (options.maxDuration) updates.maxSessionDuration = parseInt(options.maxDuration);
      
      if (options.addDirectory) {
        const config = await configService.loadConfig();
        updates.watchDirectories = [...(config.autoSave?.watchDirectories || []), options.addDirectory];
      }
      
      if (options.addPattern) {
        const config = await configService.loadConfig();
        updates.filePatterns = [...(config.autoSave?.filePatterns || []), options.addPattern];
      }
...
program
  ...
  .action(async () => {
    try {
      ...
      const { CursorIntegrationService } = await import('./services/CursorIntegrationService.js');
      const cursorService = new CursorIntegrationService(historyService, configService);
...
program
  ...
  .action(async () => {
    try {
      ...
      console.log(`✅ スキャン完了: ${status.importedSessions}/${status.foundTasks} セッションをインポート`);
    } catch (error) {
...
program
  ...
  .action(async () => {
    try {
      ...
      console.log(`最終スキャン: ${status.lastScanTime ? status.lastScanTime.toLocaleString() : '未実行'}`);
      console.log(`発見タスク数: ${status.foundTasks}`);
      console.log(`インポート済み: ${status.importedSessions}`);
      console.log(`データパス: ${config.cursor?.cursorDataPath || 'デフォルト'}`);
      console.log(`自動インポート: ${config.cursor?.autoImport ? '✅ はい' : '❌ いいえ'}`);
      console.log(`起動時インポート: ${config.cursor?.importOnStartup ? '✅ はい' : '❌ いいえ'}`);
    } catch (error) {
