...
function getClaudeDevService(): ClaudeDevIntegrationService {
  if (!claudeDevService) {
    console.error('Claude Dev統合サービス取得エラー: サービスが初期化されていません');
    console.error('サービス状態:', {
      claudeDevService: claudeDevService,
      type: typeof claudeDevService,
      timestamp: new Date().toISOString()
    });
    throw new Error('Claude Dev統合サービスが初期化されていません。サーバーを再起動してください。');
  }
 ...
 * メインサーバーから呼び出される
 */
export function setClaudeDevService(service: ClaudeDevIntegrationService): void {
 ...
 */
router.get('/tasks', asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const service = getClaudeDevService();
  
  // クエリパラメータの解析
  const maxTasks = req.query.limit ? parseInt(req.query.limit as string) : undefined;
 ...
 */
router.post('/integrate', asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const service = getClaudeDevService();
...
router.get('/sessions', asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  const sessions = await service.searchClaudeDevSessions(keyword as string, options);
 ...
 */
router.get('/sessions/:id', asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const service = getClaudeDevService();
 ...
 * Claude Devセッションの削除
 */
router.delete('/sessions/:id', asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const service = getClaudeDevService();
  const sessionId = req.params.id;
 ...
 * 特定タスクの詳細情報取得
 */
router.get('/task/:taskId/details', asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const service = getClaudeDevService();
 ...
 */
router.get('/task/:taskId/convert', asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const service = getClaudeDevService();
  const taskId = req.params.taskId;
  const includeEnvironment = req.query.includeEnvironment === 'true';
