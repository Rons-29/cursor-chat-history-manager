...
router.get('/sessions', asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  if (source === 'claude-dev' && claudeDevService) {
    // Claude Dev専用検索
    const sessions = await claudeDevService.searchClaudeDevSessions(keyword as string || '', {
...
router.get('/stats', asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  if (chatHistoryService) {
    try {
      ...
      result.services.chatHistory = { error: 'Failed to fetch chat history stats' }
    }
  }
  ...
  if (claudeDevService) {
    try {
      ...
      result.services.claudeDev = { error: 'Failed to fetch Claude Dev stats' }
    }
  }
  ...
  if (integrationService) {
    try {
      ...
      result.services.integration = { error: 'Failed to fetch integration stats' }
    }
  }
...
router.post('/search', asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  if (chatHistoryService && (!filters.source || filters.source === 'chat')) {
    try {
      ...
      results.push(...chatResult.sessions.map((session: any) => ({
        ...session,
        source: 'chat',
  ...
  // Claude Dev Search
  if (claudeDevService && (!filters.source || filters.source === 'claude-dev')) {
    try {
      const claudeDevResult = await claudeDevService.searchClaudeDevSessions(keyword, {
      ...
      results.push(...claudeDevResult.map(session => ({
        ...session,
        source: 'claude-dev',
