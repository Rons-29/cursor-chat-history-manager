...
class ChatHistoryService {
    ...
    async createSession(title, metadata) {
        const session = {
            id: (0, uuid_1.v4)(),
            title: title || `セッション ${(0, date_fns_1.format)(new Date(), 'yyyy-MM-dd HH:mm')}`,
            startTime: new Date(),
            messages: [],
            metadata: {
                totalMessages: 0,
    ...
    matchesFilter(session, filter) {
        ...
        // 日付範囲フィルター
        if (filter.startDate && (0, date_fns_1.isBefore)(session.startTime, filter.startDate)) {
            return false;
        }
        if (filter.endDate && (0, date_fns_1.isAfter)(session.startTime, filter.endDate)) {
            return false;
        }
        // タグフィルター
        ...
        if (filter.keyword) {
            ...
            const messageMatch = session.messages.some(msg => msg.content.toLowerCase().includes(keyword));
    ...
    async importSessions(filePath, options = {}) {
        const { overwrite = false, skipDuplicates = true, validateData = true } = options;
        const result = {
            imported: 0,
            skipped: 0,
            errors: []
            ...
            for (const sessionData of sessionsData) {
                try {
                    ...
                    if (validateData && !this.validateSessionData(sessionData)) {
                        result.errors.push(`無効なセッションデータ: ${sessionData.id || 'ID不明'}`);
                        continue;
                    }
                    // 日付文字列をDateオブジェクトに変換
                    const session = {
                    ...
                    result.errors.push(`セッション ${sessionData.id || 'ID不明'} のインポートエラー: ${error}`);
                }
            }
            // インデックスを更新
    ...
    async createBackup(backupPath) {
        ...
        const defaultBackupPath = path_1.default.join(this.config.storagePath, 'backups', `backup_${timestamp}.json`);
        ...
        // バックアップファイルを保存
        await fs_extra_1.default.writeJson(finalBackupPath, backupData, { spaces: 2 });
        // ファイルサイズを取得
    ...
    async performAutoBackup() {
        ...
        try {
            ...
            if (await fs_extra_1.default.pathExists(lastBackupPath)) {
                ...
                shouldBackup = hoursSinceLastBackup >= (this.config.backupInterval || 24);
            }
            if (!shouldBackup) {
                return false;
            }
    ...
    async cleanupOldBackups() {
        ...
        try {
            ...
            const backupFiles = files
                ...
                stat: fs_extra_1.default.statSync(path_1.default.join(backupDir, file))
            }))
