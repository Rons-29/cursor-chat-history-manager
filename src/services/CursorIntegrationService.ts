...
export class CursorIntegrationService extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    if (!existsSync(config.cursorDataPath)) {
      throw new Error(`Cursorデータパスが見つかりません: ${config.cursorDataPath}`);
    }
  ...
  async scanAndImport(): Promise<void> {
    ...
    try {
      ...
      for (const taskDir of taskDirs) {
        const historyFile = path.join(tasksPath, taskDir, 'api_conversation_history.json');
        ...
        if (existsSync(historyFile)) {
          try {
            ...
            console.error(`チャット履歴のインポートに失敗: ${historyFile}`, error);
          }
        }
      }
      ...
      console.log(`Cursorチャット履歴のスキャン完了: ${imported}/${this.foundTasks} セッションをインポート`);
   ...
   * Cursorのチャット履歴データを解析
   */
  private parseCursorChatData(data: any, filePath: string): CursorChatData | null {
    try {
  ...
  private async createNewSession(cursorChat: CursorChatData): Promise<void> {
    const session = await this.chatHistoryService.createSession(
      `Cursor Task: ${cursorChat.metadata?.taskId || 'Unknown'}`,
      {
        tags: [
          ...
          ...(cursorChat.metadata?.projectPath ? [`project:${path.basename(cursorChat.metadata.projectPath)}`] : [])
        ],
        source: 'cursor',
    ...
    console.log(`Cursorセッションをインポート: ${session.id} (${cursorChat.messages.length} メッセージ)`);
  }

  /**
   * 既存セッションを更新
   */
  private async updateExistingSession(sessionId: string, cursorChat: CursorChatData): Promise<void> {
    ...
    if (newMessages.length > 0) {
      ...
      console.log(`Cursorセッションを更新: ${sessionId} (+${newMessages.length} メッセージ)`);
    }
  }
  ...
  getStatus(): CursorIntegrationStatus {
    return {
      ...
      watchedDirectories: this.watchers.length > 0 ? ['Cursor tasks directory'] : []
    };
  }
} 