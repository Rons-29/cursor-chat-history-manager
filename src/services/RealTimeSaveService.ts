...
export class RealTimeSaveService extends EventEmitter {
  ...
  private async createNewSession(title?: string): Promise<void> {
    const sessionTitle =
      title || `リアルタイムセッション - ${new Date().toLocaleString('ja-JP')}`

    const session = await this.chatHistoryService.createSession(sessionTitle, {
      tags: ['realtime', 'auto-capture'],
      projectId: undefined,
      userId: undefined,
   ...
   * セッション変更を検出
   */
  private async detectSessionChange(
    content: string
