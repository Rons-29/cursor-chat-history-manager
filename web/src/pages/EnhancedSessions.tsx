...
const enhanceSessionMetadata = (session: any) => {
  // セッション内容からキーワード抽出
  const content = [
    session.title || '',
    ...(session.messages || []).map((msg: any) => msg.content || '')
  ].join(' ').toLowerCase()

  // カテゴリ判定（実際のコンテンツベース）
  const getCategory = () => {
    if (content.includes('エラー') || content.includes('バグ') || content.includes('debug')) return 'debugging'
    if (content.includes('テスト') || content.includes('test')) return 'testing'
    if (content.includes('質問') || content.includes('相談') || content.includes('どう')) return 'consultation'
    if (content.includes('実装') || content.includes('コード') || content.includes('機能')) return 'development'
    return 'development'
  }
  ...
  const getComplexity = () => {
    ...
    const hasError = content.includes('エラー') || content.includes('error')
    
    if (messageCount > 10 || (hasCode && hasError)) return 'complex'
    if (messageCount > 3 || hasCode || hasError) return 'medium'
    return 'simple'
  }
  ...
  const getQuestionType = () => {
    ...
    if (content.includes('実装') || content.includes('作成')) return 'implementation'
    if (content.includes('修正') || content.includes('エラー')) return 'debugging'
    return 'consultation'
  }

  // 解決状況判定
  const getResolved = () => {
    const lastMessages = (session.messages || []).slice(-3).map((m: any) => m.content || '').join(' ').toLowerCase()
    return lastMessages.includes('ありがと') || lastMessages.includes('解決') || lastMessages.includes('完了') || lastMessages.includes('できました')
  }
  ...
  const generateTags = () => {
    ...
    if (content.includes('データベース') || content.includes('db')) tags.push('Database')
    if (content.includes('ui') || content.includes('コンポーネント')) tags.push('UI')
    if (content.includes('エラー') || content.includes('バグ')) tags.push('バグ修正')
    if (content.includes('機能') || content.includes('実装')) tags.push('機能開発')
    
    // メタデータのソースも追加
    if (session.metadata?.source) tags.push(session.metadata.source)
    
    // 最低限のタグを保証
    if (tags.length === 0) tags.push('AI対話')
  ...
  // タイトル改善（実際のコンテンツベース）
  const improveTitle = () => {
    const originalTitle = session.title || 'セッション'
    
    // 既に意味のあるタイトルの場合はそのまま
    if (!originalTitle.includes('Cursor') && !originalTitle.includes('Session') && originalTitle.length > 10) {
    ...
    if (firstMessage.length > 10) {
      const shortened = firstMessage.substring(0, 40).trim()
      return shortened + (firstMessage.length > 40 ? '...' : '')
    }
  ...
  return {
    ...
    topic: generateTags()[0] || 'AI対話',
    category: getCategory(),
    questionType: getQuestionType(),
    complexity: getComplexity(),
    resolved: getResolved(),
    followUpNeeded: !getResolved() && (session.messages?.length || 0) > 2,
    totalMessages: session.messages?.length || 0,
    codeBlocks: (content.match(/```/g) || []).length / 2, // コードブロック数
    autoGeneratedTags: generateTags(),
    summary: session.metadata?.description || `${getCategory()}に関する${session.messages?.length || 0}件のメッセージ`,
    keywords: generateTags().slice(0, 3),
    titleGeneration: {
      confidence: improveTitle() !== session.title ? 'high' : 'medium' as 'high' | 'medium' | 'low',
      autoGenerated: improveTitle() !== session.title
    },
    improvedTitle: improveTitle()
  }
}
